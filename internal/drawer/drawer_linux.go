...
// fip_lc_numeric reads the process LC_NUMERIC category; fip_keep_c_numeric
// pulls it back to "C".
//
// gtk_init calls setlocale(LC_ALL, "") to adopt the user's locale, and that is
// process-global: on an en_US/fr_FR desktop it leaves LC_NUMERIC non-"C".
// libmpv REFUSES to create a handle in that state (mpv/client.h: mpv_create
// returns NULL when "LC_NUMERIC is not set to C", and mpv prints "Non-C locale
// detected"). That silently killed the audio fondu once the panel started
// pre-warming at startup: every incoming crossfade handle failed to init and the
// zap fell back to a hard cut. Only LC_NUMERIC is pulled back, so GTK and
// WebKit keep the user's collation/time/message locale.
static const char *fip_lc_numeric(void) { return setlocale(LC_NUMERIC, NULL); }
static void fip_keep_c_numeric(void) { setlocale(LC_NUMERIC, "C"); }
...
static GtkWidget *fip_build(WebKitWebView **out_view, const char *html, int width, int height) {
	...
	// The window title is what the shell shows in alt-tab and the window list:
	// the app's name, not the part's name. The dev harnesses keep their own
	// branded titles (« maquette », « autotest ») so a mock is never mistaken
	// for the real panel.
	gtk_window_set_title(GTK_WINDOW(win), "le FIPindicateur");
...
// NumericLocale reports the process LC_NUMERIC category. The panel is the only
// component that perturbs it (gtk_init adopts the user's locale) and the only one
// that restores it, so `drawer --selftest` asserts it comes back to "C": libmpv
// refuses to open a second stream otherwise and the audio fondu silently
// degrades to a hard cut.
func NumericLocale() string { return C.GoString(C.fip_lc_numeric()) }
...
func (d *Drawer) ensureStarted() error {
	d.startOnce.Do(func() {
		...
		go func() {
			...
			// gtk_init just took the process locale from the environment;
			// libmpv cannot create a handle unless LC_NUMERIC is "C", so pull
			// that one category back immediately (see fip_keep_c_numeric).
			numericFromGTK := C.GoString(C.fip_lc_numeric())
			C.fip_keep_c_numeric()
			tInit := time.Now()
			...
			d.view = view
			// Re-assert after the WebKit view exists: whoever perturbs the
			// locale last must not win over the player.
			numericFromWebKit := C.GoString(C.fip_lc_numeric())
			C.fip_keep_c_numeric()
			if got := C.GoString(C.fip_lc_numeric()); got != "C" {
				log.Printf("drawer: LC_NUMERIC reste %q après restauration: libmpv refusera un second flux, le fondu sera une coupure sèche", got)
			}
			timing("panneau: LC_NUMERIC posé par gtk_init=%q, par WebKit=%q, restauré à « C » pour libmpv",
				numericFromGTK, numericFromWebKit)
			d.builtT = time.Now()
