...
func TestExplicitTastes(t *testing.T) {
	prf := []prefs.Entry{
		{Artist: "A", Title: "1", Verdict: prefs.VerdictLike, TS: at(1)},
		{Artist: "B", Title: "2", Verdict: prefs.VerdictLike, TS: at(3)},
		{Artist: "C", Title: "3", Verdict: prefs.VerdictDislike, TS: at(2)},
	}
...
func TestItemsCapAtTwenty(t *testing.T) {
	var prf []prefs.Entry
	for i := 0; i < 25; i++ {
		prf = append(prf, prefs.Entry{Artist: "A", Verdict: prefs.VerdictLike, TS: at(i)})
	}
	r := Build(nil, nil, prf, nil, base)
